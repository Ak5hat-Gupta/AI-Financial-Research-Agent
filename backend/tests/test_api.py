...
def test_ratios_endpoint(auth_client):
    r = auth_client.post("/api/ratios?live=false", json={"ticker": "MSFT"})
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_competitors_endpoint(auth_client):
    r = auth_client.post("/api/competitors?live=false", json={"ticker": "AAPL", "peers": ["MSFT"]})
    assert r.status_code == 200, r.text
    assert len(r.json()["peers"]) >= 2


def test_sentiment_endpoint(auth_client):
    r = auth_client.post("/api/sentiment?live=false", json={"ticker": "TSLA"})
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_document_upload_and_chat(auth_client):
    ...
    r = auth_client.post("/api/documents", files=files, data={"ticker": "ACME", "company": "Acme Corp"})
    ...
    r2 = auth_client.post("/api/chat/ask", json={"message": "What was revenue?", "document_id": doc["id"]})
    assert r2.status_code == 200, r2.text
    assert r2.json()["answer"]


def test_portfolio_flow(auth_client):
    r = auth_client.post("/api/portfolio/holdings", json={"ticker": "AAPL", "shares": 10, "cost_basis": 150})
    assert r.status_code == 201
    r2 = auth_client.get("/api/portfolio/recommendations?live=false")
    assert r2.status_code == 200, r2.text
    body = r2.json()
